 ...
 */
public class TestOrderRegEndMgn {
...
//fill out register form
        BillingMgnPage.enterEmail("triggmine01@gmail.com", driver);//set email
        BillingMgnPage.fillBilling("test", "test", "test 2", "test", "12345",
                "8005558789", driver); //fill main billing inf
...
//GetBuyerId
        ...
        assertEquals(data.get("BuyerEmail").toString(), "triggmine01@gmail.com");
...
//CreateReplaceBuyerInfo
        ...
        assertEquals(buyerId, data.get("BuyerId"));//check "BuyerId"
        assertTrue(data.containsKey("BuyerRegEnd"));//check BuyerRegEnd
        assertEquals(data.get("BuyerEmail").toString(), "triggmine01@gmail.com");
        assertEquals(token, jsonObjectHashMap.get("Token"));//check Token is the same for each action
